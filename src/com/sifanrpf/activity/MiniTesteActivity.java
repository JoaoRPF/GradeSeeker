...
public class MiniTesteActivity extends ActionBarActivity implements OnItemSelectedListener {
	...
	protected void onCreate(Bundle savedInstanceState){
		...
		setContentView(R.layout.mini_teste);
		...
		spNumeroTestes = (Spinner) findViewById(R.id.spinnerNumeroMiniTestes);
		...
		final EditText percentagemMiniTeste = (EditText) findViewById(R.id.percentagemMiniTeste);
		percentagemMiniTeste.setFilters(new InputFilter[]{ new InputFilterMaxMin("1", "100")});
		
		percentagemMiniTeste.setOnFocusChangeListener(new View.OnFocusChangeListener() {
	        @Override
		...
		listViewPercs.setOnItemClickListener(new OnItemClickListener() {
			public void onItemClick(AdapterView<?> parent, View view, final int position, long id){
				AlertDialog.Builder requestPercentagem = new AlertDialog.Builder(MiniTesteActivity.this);
		...
		final Button button = (Button) findViewById(R.id.gravarMiniTeste);
		button.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				String percentagemTesteStr = percentagemMiniTeste.getText().toString();
				if(!percentagemTesteStr.equals("")){
					if(verificaPercentagens()){
						...
						Toast.makeText(getBaseContext(), "O conjunto dos Mini Testes tem que valer 100%"
	        					, Toast.LENGTH_LONG).show();
					}
				}
				else{
					Toast.makeText(getBaseContext(), "Por favor, insira a % que os Mini Testes valem."
        					, Toast.LENGTH_LONG).show();
				}
			}
		});		
	...
	public void criaMiniTeste(){
		MiniTeste teste = miniTesteDataSource.createMiniTeste(nomeDisciplina, numeroTestes, percentagemTotal);
	}
	...
	public void gravaCotacaoTestes(){
		if(flagCotacoes==0){
			...
			miniTesteDataSource.gravaCotacaoTestes(nomeDisciplina, numeroTestes, cotacoes);
		}
	}
	...
	public void gravaNovaPercentagem(){
		miniTesteDataSource.alteraCotacaoTeste(nomeDisciplina, numeroTestes, percsTemporarias);
	}
